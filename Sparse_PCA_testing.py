...
offset_list = np.arange(24, 36, 2)
upto_list = np.arange(54, 66, 2)
sparsePCA_alpha = [20, 50, 80]
# sparsePCA_ridge_alpha = [0.0001, 0.01, 0.1, 1, 10]
MeanShift_alpha = [10, 50, 100]
CompNumb = [2, 3]
    ...
    for sparsePCA_alpha_ind in sparsePCA_alpha:
        ...
        for CompNumber in CompNumb:
            for MeanShift_alpha_ind in MeanShift_alpha:
                ...
                O.CombinedMeanShift(0.3, MeanShift_alpha_ind, PrincComp=scorePCA[:CompNumber, :], mbf=10)
                ...
                plt.savefig("clusters/" + str(offset) + "to" + str(upto) + "_alpha" + str(sparsePCA_alpha_ind) +
                            "_CompNumber" + str(CompNumber) + "_meanshift" + str(MeanShift_alpha_ind) + ".png", bbox_inches='tight')
                plt.clf()
...
plt.xlabel('Times')
plt.xlim(0, len(fp_mean))
dim = np.arange(0, len(fp_mean), 1)
plt.plot(fp_mean, 'ro', color='r', linewidth=1.0)
plt.xticks(dim)
plt.grid()
plt.savefig("fn_fp/False positives Trend.png", bbox_inches='tight')
plt.clf()
...
plt.ylabel('Average values')
plt.xlabel('Times')
plt.xlim(0, len(fn_mean))
dim = np.arange(0, len(fn_mean), 1)
plt.plot(fn_mean, 'ro', color='r', linewidth=1.0)
plt.xticks(dim)
plt.grid()
plt.savefig("fn_fp/False negatives Trend.png", bbox_inches='tight')
plt.clf()
...
plt.ylabel('Average values')
plt.xlabel('Times')
plt.xlim(0, len(f1_mean))
dim = np.arange(0, len(f1_mean), 1)
plt.plot(f1_mean, 'ro', color='r', linewidth=1.0)
plt.xticks(dim)
plt.grid()
plt.savefig("f1/F-measure Trend.png", bbox_inches='tight')
plt.clf()
# plt.show()
